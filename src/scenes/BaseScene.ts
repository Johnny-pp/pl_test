...
const JOB_LABELS: Record<BaseJob, string> = { planting: "种植", mining: "采矿", lumbering: "伐木", generating: "发电" };
const FACILITY_LABELS: Record<FacilityId, string> = { warehouse: "仓库", farm: "农圃", workshop: "工坊" };
const RESOURCE_LABELS = { wood: "木材", stone: "石料", food: "食物", fiber: "纤维", crystal: "晶体" } as const;
...
export class BaseScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.add.text(18, 18, "< 返回图鉴", {
      fontFamily: "sans-serif", fontSize: "18px", color: "#4fc3f7",
    }).setInteractive({ useHandCursor: true }).on("pointerdown", () => this.scene.start("DexScene"));
    this.add.text(450, 28, "远征基地", {
  ...
  private render() {
    ...
    const resourceLine = Object.entries(this.save.base.resources)
      ...
      .join("  ·  ");
    this.addToContent(this.add.text(450, 70, resourceLine, {
      fontFamily: "sans-serif", fontSize: "15px", color: "#d8def8",
    }).setOrigin(0.5));
    this.addToContent(this.add.text(450, 96,
    ...
    (Object.keys(FACILITY_LABELS) as FacilityId[]).forEach((facility, index) => {
      ...
      this.addToContent(this.add.text(x, 130, `${FACILITY_LABELS[facility]} Lv.${this.save.base.facilities[facility]}`, {
        fontFamily: "sans-serif", fontSize: "17px", color: "#ffffff",
      }).setOrigin(0.5));
      this.addToContent(this.makeButton(x, 160, 130, "升级设施", () => this.upgrade(facility)));
    });

    this.addToContent(this.add.text(62, 205, "制造", { fontFamily: "sans-serif", fontSize: "19px", color: "#ffffff" }));
    this.addToContent(this.makeButton(230, 218, 230, this.recipeLabel("capture-orb", "捕获器"), () => this.craft("capture-orb")));
    this.addToContent(this.makeButton(515, 218, 230, this.recipeLabel("healing-tonic", "治疗剂"), () => this.craft("healing-tonic")));
    this.addToContent(this.makeButton(760, 218, 120, "立即结算", () => this.settleProduction()));

    const message = this.add.text(450, 258, this.message, {
      fontFamily: "sans-serif", fontSize: "14px", color: "#ffd54f",
    }).setOrigin(0.5);
    this.addToContent(message);
    this.addToContent(this.add.text(42, 286, "岗位分配（每 5 秒及离线期间自动生产）", {
      fontFamily: "sans-serif", fontSize: "18px", color: "#ffffff",
    }));

    if (this.save.ownedPals.length === 0) {
      this.addToContent(this.add.text(450, 380, "尚无可工作的幻兽，请先探索并捕获。", {
        fontFamily: "sans-serif", fontSize: "18px", color: "#9aa0c0",
      }).setOrigin(0.5));
      return;
    }
    this.save.ownedPals.forEach((instance, index) => {
      ...
      const name = this.add.text(x - 180, y - 43, `${species.name.zh} · ${assignment ? JOB_LABELS[assignment.job] : "休息中"}`, {
        fontFamily: "sans-serif", fontSize: "17px", color: "#ffffff",
      });
      const suitability = species.workSuitability
        .filter((work) => ["planting", "mining", "lumbering", "generating", "electricity"].includes(work.type))
        .map((work) => `${work.type === "electricity" ? "发电" : JOB_LABELS[work.type as BaseJob]} Lv.${work.level}`)
        .join(" · ") || "无基地岗位适性";
      ...
      jobs.forEach((job, jobIndex) => {
        this.addToContent(this.makeButton(x - 135 + jobIndex * 88, y + 27, 80, JOB_LABELS[job], () => this.assign(instance.uid, job)));
      });
      if (assignment) this.addToContent(this.makeButton(x + 135, y + 27, 80, "休息", () => this.unassign(instance.uid)));
    });
  }
  ...
  private availableJobs(species: (typeof pals)[number]): BaseJob[] {
    ...
    for (const work of species.workSuitability) {
      ...
      if (work.type === "planting" || work.type === "mining" || work.type === "lumbering") jobs.add(work.type);
    }
    return [...jobs];
  }

  private recipeLabel(item: CraftableItem, name: string) {
    const cost = Object.entries(CRAFT_RECIPES[item]).map(([id, amount]) => `${RESOURCE_LABELS[id as keyof typeof RESOURCE_LABELS]}${amount}`).join(" ");
    return `${name} · ${cost}`;
  }
