...
export class CompareScene extends Phaser.Scene {
  ...
  private render() {
    ...
    this.addContent(this.add.text(450, 260, "左侧相对差值", {
    ...
    STAT_ROWS.forEach(([label, value], index) => {
      ...
      this.addContent(this.add.text(72, y, label, { fontFamily: "sans-serif", fontSize: "15px", color: "#b8c0df" }));
      this.addContent(this.add.rectangle(145, y + 8, 210, 12, 0x17233e).setOrigin(0, 0.5));
      this.addContent(this.add.rectangle(145, y + 8, 210 * leftValue / max, 12, 0x4fc3f7).setOrigin(0, 0.5));
      this.addContent(this.add.text(365, y, String(leftValue), { fontFamily: "sans-serif", fontSize: "14px", color: "#ffffff" }));
      const difference = leftValue - rightValue;
      this.addContent(this.add.text(450, y, `${difference > 0 ? "+" : ""}${difference}`, {
        fontFamily: "sans-serif", fontSize: "14px", color: difference > 0 ? "#9ccc65" : difference < 0 ? "#ff8a80" : "#9aa0c0",
      }).setOrigin(0.5));
      this.addContent(this.add.text(505, y, String(rightValue), { fontFamily: "sans-serif", fontSize: "14px", color: "#ffffff" }));
      this.addContent(this.add.rectangle(545, y + 8, 210, 12, 0x17233e).setOrigin(0, 0.5));
      this.addContent(this.add.rectangle(545, y + 8, 210 * rightValue / max, 12, 0xff8a65).setOrigin(0, 0.5));
    });
  }

  private renderHeader(pal: Pal, x: number, side: "left" | "right") {
    this.addContent(addPalPortrait(this, pal.id, x, 135, 145));
    this.addContent(this.add.text(x, 215, pal.name.zh, {
      fontFamily: "sans-serif", fontSize: "22px", color: "#ffffff",
    }).setOrigin(0.5));
    const element = pal.elements[0] ?? "neutral";
    this.addContent(this.add.text(x, 242, pal.elements.map((item) => ELEMENT_LABELS[item]).join("/"), {
      fontFamily: "sans-serif", fontSize: "14px", color: Phaser.Display.Color.IntegerToColor(ELEMENT_COLORS[element]).rgba,
    }).setOrigin(0.5));
  ...
  private makeButton(x: number, y: number, label: string, action: () => void) {
    ...
    const text = this.add.text(x, y, label, { fontFamily: "sans-serif", fontSize: "30px", color: "#ffffff" }).setOrigin(0.5);
