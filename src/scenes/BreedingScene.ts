...
export class BreedingScene extends Phaser.Scene {
  ...
  private render() {
    ...
    const offspring = firstSpecies && secondSpecies ? previewOffspring(firstSpecies, secondSpecies, pals) : undefined;

    this.addContent(this.add.text(34, 66,
      `父母 A：${firstSpecies?.name.zh ?? "未选择"}    父母 B：${secondSpecies?.name.zh ?? "未选择"}`,
      { fontFamily: "sans-serif", fontSize: "16px", color: "#d8def8" }
    ));
    this.addContent(this.add.text(34, 94,
      offspring ? `预计后代：${offspring.name.zh} · 消耗食物 ${BREEDING_FOOD_COST}` : "依次点击两个不同个体以预览后代",
    ...
    this.addContent(this.add.text(34, 124, this.message, {
      fontFamily: "sans-serif", fontSize: "14px", color: "#ffd54f",
    }));

    this.addContent(this.add.text(34, 154, "选择父母", {
      fontFamily: "sans-serif", fontSize: "19px", color: "#ffffff",
    }));
    if (this.save.ownedPals.length < 2) {
      this.addContent(this.add.text(280, 250, "至少需要两个已拥有的幻兽", {
    ...
    this.addContent(this.add.text(722, 116, `孵化队列 ${this.save.breedingEggs.length}/4`, {
      fontFamily: "sans-serif", fontSize: "19px", color: "#ffffff",
    }).setOrigin(0.5));
    if (this.save.breedingEggs.length === 0) {
      this.addContent(this.add.text(722, 190, "暂无待孵化的蛋", {
        fontFamily: "sans-serif", fontSize: "16px", color: "#68718e",
      }).setOrigin(0.5));
    }
    this.save.breedingEggs.forEach((egg, index) => {
      ...
      const panel = this.add.rectangle(722, y, 282, 92, 0x16213e).setStrokeStyle(2, QUALITY_COLORS[egg.quality]);
      const title = this.add.text(596, y - 35, `${QUALITY_LABELS[egg.quality]}蛋 · ${species?.name.zh ?? "未知"}`, {
        fontFamily: "sans-serif", fontSize: "16px", color: "#ffffff",
      });
      const inherited = egg.passiveSkillIds.map((id) => passiveSkillsById.get(id)?.name.zh ?? id).join("、") || "无继承被动";
  ...
  private startBreeding() {
    ...
    const result = breed(this.save, this.parentA, this.parentB, pals, passiveSkills.map((skill) => skill.id));
    if (result.error) {
      const labels = { "same-parent": "不能选择同一个体", "missing-parent": "父母不存在", "missing-species": "物种数据缺失", "insufficient-food": "食物不足", "queue-full": "孵化队列已满" };
