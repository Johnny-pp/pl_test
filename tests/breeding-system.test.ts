...
function species(id: number, power: number, rarity = 1): Pal {
  return {
    id, name: { zh: `幻兽${id}`, en: `Pal${id}` }, rarity, size: "small", elements: ["neutral"], catchRate: 50, foodAmount: 2,
    ...
    workSuitability: [], activeSkills: ["quick-strike"], breeding: { power, parents: [] },
  };
}
...
function parents() {
  ...
  save = addCapturedPal(save, createPalInstance(pool[0], () => "a", undefined, ["windstep"]));
  save = addCapturedPal(save, createPalInstance(pool[2], () => "b", undefined, ["stonehide"]));
  return save;
}
...
test("配种消耗食物并将蛋加入队列", () => {
  const save = parents();
  const result = breed(save, "a", "b", pool, ["windstep"], () => 0, 1000, () => "egg-1");
...
test("未到时间不能孵化，到时生成带继承被动的新个体", () => {
  const bred = breed(parents(), "a", "b", pool, [], () => 0, 1000, () => "egg-1").save;
