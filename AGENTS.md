...
- 阶段十七已完成：芦灯港聚落 NPC 对话/商店/治疗、星币与掉落物经济、6 条不同目标类型支线、5 种探索能力机关门与隐藏宝箱、可见精英训练者挑战（可配置重战）、任务页主线/支线切换与存档 v9 迁移，均通过自动化与真实 Firefox 浏览器验收
- 阶段十八已完成：基地改为生产/布局/科技/加工/订单五页签，6×4 格子布局、设施占地/邻接、5 节点科技树（地区/首领/基地等级解锁）、矿石→金属→装备/高级捕获器多级加工链、4 类可重复订单与存档 v10 迁移，均通过自动化与真实 Firefox 浏览器验收
- 阶段十九已完成：10 层试炼塔（逐层增强、元素/人数限制、阶段奖励）、4 位主线首领强化重战、按日历种子生成的每日/每周委托、战斗评分最佳纪录、17 项成就与可装备称号、随机遭遇/限制捕获/永久倒下三项新周目开关，以及存档 v11 迁移，均通过自动化与真实 Firefox 浏览器验收
- 阶段二十已完成：设置系统（音量/动画/文字速度/减少动态效果/高对比色盲辅助/键位）、Web Audio 原创音效与环境音乐、按进度激活的可跳过新手引导、3 个存档槽与自动备份/恢复点、数据维护工具、PWA 基础支持与开源/隐私/第三方声明文档，均通过自动化与真实 Firefox 浏览器验收
